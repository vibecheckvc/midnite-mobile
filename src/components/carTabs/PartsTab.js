// src/components/carTabs/PartsTab.js
...
export default function PartsTab({ car, supabase, onReload }) {
  ...
  const openAdd = () => { setEditing(null); setF({ name:"", category:"", cost:"" }); setShow(true); };
  const openEdit = (r) => { setEditing(r); setF({ name:r.name||"", category:r.category||"", cost: r.cost!=null ? String(r.cost) : "" }); setShow(true); };

  const save = async () => {
    if (!f.name.trim()) return Alert.alert("Missing", "Part name required.");
    const payload = { car_id: car.id, name: f.name.trim(), category: f.category?.trim() || null, cost: f.cost ? Number(f.cost) : null };
    if (editing) {
  ...
  const toggleInstalled = async (r) => {
    setRows(prev => prev.map(x => x.id===r.id ? { ...x, installed: !x.installed } : x));
    const { error } = await supabase.from(TABLE).update({ installed: !r.installed }).eq("id", r.id);
    if (error) Alert.alert("Error", error.message); else onReload?.();
  };

  const del = (id) => Alert.alert("Delete part?", "This cannot be undone.", [
    ...
    <View style={styles.card}>
      <View style={styles.header}>
        ...
        <View style={{ flexDirection:"row", gap:8 }}>
          <TouchableOpacity onPress={()=>toggleInstalled(item)} style={[styles.chip, item.installed && styles.chipOn]}>
      ...
      <Text style={styles.meta}>{item.category || "—"} {item.cost!=null ? `· $${Number(item.cost).toLocaleString()}` : ""}</Text>
    </View>
  );
  ...
  return (
    <View style={styles.wrap}>
      ...
      <View style={{ paddingHorizontal:16, paddingBottom:16 }}>
        <TouchableOpacity onPress={openAdd} style={styles.primary}><Ionicons name="add" size={18} color="#fff"/><Text style={styles.primaryTxt}>Add Part</Text></TouchableOpacity>
      </View>
      ...
      <Modal visible={show} animationType="slide" presentationStyle="pageSheet" onRequestClose={()=>setShow(false)}>
        <View style={styles.modal}>
          ...
          <View style={{ padding:16 }}>
            ...
            <Field label="Category" value={f.category} onChangeText={(t)=>setF({...f,category:t})}/>
            <Field label="Cost" value={f.cost} onChangeText={(t)=>setF({...f,cost:t})} keyboardType="numeric"/>
          </View>
        </View>
      </Modal>
    </View>
...
const styles = StyleSheet.create({
  ...
  chip:{ borderWidth:1, borderColor:"rgba(255,255,255,0.18)", paddingVertical:4, paddingHorizontal:10, borderRadius:999, backgroundColor:"rgba(255,255,255,0.06)" },
